...
my %SUPPORTED = map { $_ => 1 } qw(
    ...
    mediainfo openexr openjpeg zlib zstd
);
...
sub build_openexr {
    my ($openexr_version, $imath_version) = ('3.4.15', '3.2.3');

    download_extract(
        "https://github.com/AcademySoftwareFoundation/Imath/archive/refs/tags/v$imath_version.zip",
        "Imath-$imath_version.zip",
    );
    cmake_project(
        File::Spec->catdir($BUILD_ROOT, "Imath-$imath_version"),
        File::Spec->catdir($BUILD_ROOT, 'imath-build'),
        '-DBUILD_SHARED_LIBS=OFF', '-DBUILD_TESTING=OFF',
        '-DIMATH_INSTALL=ON', '-DIMATH_INSTALL_PKG_CONFIG=ON',
        '-DPYTHON=OFF', '-DPYBIND11=OFF', '-DBUILD_WEBSITE=OFF',
    );

    download_extract(
        "https://github.com/AcademySoftwareFoundation/openexr/archive/refs/tags/v$openexr_version.zip",
        "openexr-$openexr_version.zip",
    );
    cmake_project(
        File::Spec->catdir($BUILD_ROOT, "openexr-$openexr_version"),
        File::Spec->catdir($BUILD_ROOT, 'openexr-build'),
        "-DCMAKE_PREFIX_PATH=$PREFIX", '-DBUILD_SHARED_LIBS=OFF',
        '-DBUILD_TESTING=OFF', '-DOPENEXR_BUILD_LIBS=ON',
        '-DOPENEXR_INSTALL=ON', '-DOPENEXR_BUILD_TOOLS=OFF',
        '-DOPENEXR_INSTALL_TOOLS=OFF',
        '-DOPENEXR_INSTALL_DEVELOPER_TOOLS=OFF',
        '-DOPENEXR_BUILD_EXAMPLES=OFF', '-DOPENEXR_BUILD_PYTHON=OFF',
        '-DOPENEXR_BUILD_OSS_FUZZ=OFF', '-DOPENEXR_INSTALL_DOCS=OFF',
        '-DBUILD_WEBSITE=OFF', '-DOPENEXR_USE_TBB=OFF',
...
sub build_mediainfo {
    ...
    cmake_project(
        File::Spec->catdir($BUILD_ROOT, 'MediaInfoLib', 'Project', 'CMake'),
        File::Spec->catdir($BUILD_ROOT, 'build'),
