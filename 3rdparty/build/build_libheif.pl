...
sub patch_x265_cmake {
    my ($source) = @_;
    my $cmake_file = File::Spec->catfile($source, 'CMakeLists.txt');

    open my $input, '<', $cmake_file
        or fail("Cannot open $cmake_file: $!");
    local $/;
    my $contents = <$input>;
    close $input or fail("Cannot close $cmake_file: $!");

    my @replacements = (
        [
            "# vim: syntax=cmake\n",
            "# vim: syntax=cmake\ncmake_minimum_required (VERSION 3.10)\n",
            'move cmake_minimum_required before project',
        ],
        [
            "cmake_policy(SET CMP0025 OLD) # report Apple's Clang as just Clang",
            "cmake_policy(SET CMP0025 NEW) # report Apple's Clang as AppleClang",
            'use the required CMP0025 behavior',
        ],
        [
            'cmake_policy(SET CMP0054 OLD)',
            'cmake_policy(SET CMP0054 NEW)',
            'use the required CMP0054 behavior',
        ],
        [
            "project (x265)\ncmake_minimum_required (VERSION 2.8.8) # OBJECT libraries require 2.8.8\n",
            "project (x265)\n",
            'remove the obsolete minimum-version declaration',
        ],
        [
            'if(${CMAKE_CXX_COMPILER_ID} STREQUAL "Clang")',
            'if(CMAKE_CXX_COMPILER_ID MATCHES "^(Apple)?Clang$")',
            'recognize AppleClang with CMP0025 NEW',
        ],
    );

    for my $replacement (@replacements) {
        my ($old, $new, $description) = @{$replacement};
        my $count = ($contents =~ s/\Q$old\E/$new/);
        fail("Cannot $description in $cmake_file") if $count != 1;
    }

    open my $output, '>', $cmake_file
        or fail("Cannot write $cmake_file: $!");
    print {$output} $contents
        or fail("Cannot write $cmake_file: $!");
    close $output or fail("Cannot close $cmake_file: $!");
    print "[PATCH] x265 CMake compatibility for CMake 4.x\n";
}
